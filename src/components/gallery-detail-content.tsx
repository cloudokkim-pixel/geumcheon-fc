  ...
  return (
    <>
      <div className="mb-8 rounded-[14px] bg-[#1c1c1e] p-6 sm:p-8">
        <div className="flex flex-wrap items-center gap-3 text-sm text-[#888890]">
        ...
        <p className="mt-5 max-w-3xl text-sm leading-8 text-[#c0c0c5] sm:text-base">{item.description}</p>
      </div>
      ...
      <div className="grid gap-8 lg:grid-cols-[1.1fr_0.9fr]">
        ...
        <aside className="rounded-[14px] bg-[#1c1c1e] p-6 sm:p-8">
          ...
          <div className="mt-6 space-y-4 text-sm text-[#c0c0c5]">
            <div>
              <p className="text-[#888890]">{t.category}</p>
              <p className="mt-1">{getGalleryCategoryLabel(locale, item.category)}</p>
            </div>
            <div>
              <p className="text-[#888890]">{t.createdAt}</p>
              <p className="mt-1">{item.createdAt.slice(0, 10)}</p>
            </div>
            <div>
              <p className="text-[#888890]">{t.updatedAt}</p>
              <p className="mt-1">{item.updatedAt.slice(0, 10)}</p>
            </div>
            <div>
              <p className="text-[#888890]">{t.imageCount}</p>
      ...
      <div className="mt-10 grid gap-4 sm:grid-cols-2">
        <div className="rounded-[14px] bg-[#1c1c1e] p-5">
          <p className="text-xs uppercase tracking-[0.2em] text-[#888890]">{t.previous}</p>
          {previous ? (
            <Link href={`/gallery/${previous.slug}`} className="mt-3 block text-lg font-semibold text-[#f5f5f7]">{previous.title}</Link>
          ) : (
            <p className="mt-3 text-[#888890]">{t.previousEmpty}</p>
          )}
        </div>
        <div className="rounded-[14px] bg-[#1c1c1e] p-5">
          <p className="text-xs uppercase tracking-[0.2em] text-[#888890]">{t.next}</p>
          {next ? (
            <Link href={`/gallery/${next.slug}`} className="mt-3 block text-lg font-semibold text-[#f5f5f7]">{next.title}</Link>
          ) : (
            <p className="mt-3 text-[#888890]">{t.nextEmpty}</p>
          )}
        </div>
      </div>
    </>
